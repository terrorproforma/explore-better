...
A fast dual-pane file manager for Windows power users, with tabs, preview, transactional file operations, scripting, reversible Explorer integration, and a hierarchical Disk Map.
...
Download the current Windows x64 installer from the [latest GitHub release](https://github.com/terrorproforma/explore-better/releases/latest). The installer runs per-user by default, includes the native filesystem helper, and creates optional Desktop and Start Menu shortcuts.

The initial public preview is not Authenticode-signed. Windows SmartScreen may show an unrecognized-app warning; verify the SHA-256 value published with the release before choosing **More info** and **Run anyway**. Explorer integration is optional and reversible from the Integration Center.
...
- Adaptive Command Center for built-in actions, saved trusted tools, and saved trusted scripts, with fuzzy acronym matching, category ranking, persistent pins, recent-command history, hotkey badges, and keyboard selection.
...
- Precision-workstation chrome with Lucide Search, Disk Map, Operations, Command, and Focus controls in the top bar; stable 34px per-pane create/rename/copy/move/recycle icons with live directional tooltips; local `More` menus for advanced pane controls; symbol-based navigation/view/layout controls; and an adaptive command shelf that anchors active-pane status and workspace modes while wrapping customized actions into every row the user exposes by resizing it. Focus mode removes navigator, preview, roots, and secondary brand text while retaining one-click primary commands and every command through `Ctrl+P`.
- Adaptive horizontal-pane chrome compresses tabs, navigation, actions, and column headers into a 134 px stack; breadcrumbs remain available from the trail button as an overlay that does not resize the file list. The focused geometry proof gives each 368 px pane a 232 px listing area with zero toolbar overflow.
- Explicit dual-pane safety state marks the active operation `SOURCE` and opposite `TARGET`, follows pane focus immediately, and gives Copy/Move a layout-aware direction arrow. Selection-dependent actions stay disabled with accessible reasons until the pane has a valid selection, eliminating misleading no-op clicks.
- Persistent `Focus` workspace mode (`F9`) that temporarily removes Navigator and Preview from the grid, expands file-pane area by more than 50% in the desktop verification fixture, works across vertical/horizontal/single layouts, and restores the existing panel sizes without discarding functionality. Navigator and Preview can also be collapsed independently from their headers, restored from permanent Lucide panel controls in the workspace strip, toggled through Command, and persisted across restart; the focused 1280px geometry fixture gives each horizontal pane at least 25% more width when Navigator is hidden while keeping every panel available.
...
- Resilient startup restores the last session when possible and automatically climbs to the nearest existing ancestor when a previously saved folder has been moved or deleted. Explicit shell/URL targets remain authoritative and report their error instead of being silently rewritten.
...
- Folder Size scanning in panes, plus a first-class top-bar `Disk Map` that opens directly into a full-window map workspace with logical and allocated size totals, top folders/files/extensions, type categories, a live legend, cold in-flight scan coalescing, a visible Cancel flow for abandoned scans, warm-cache repeat scans that preserve the rendered map, and operation/script invalidation. Local Windows analysis uses one bounded native traversal, parallel exact allocation lookup, and compact root-relative column transport instead of a second Node filesystem walk. Its squarified hierarchical treemap keeps nested folders and files together; `Size by` switches actual rectangle area between logical and allocated bytes, while `Color by` switches between file type and stable top-folder branches. `Overview` restores the complete folder/type tables without rescanning. Map clicks pin a selection, double-click drills into a folder or opens a file, breadcrumbs plus Root/Up/Focus/Open commands make navigation explicit, and the same workflow is keyboard accessible. The Properties and size audit dialog adds recursive folder totals, timestamps, optional file hashes, and bounded Path Health diagnostics for local, UNC, and mapped-drive targets, plus native Windows Properties sheets through `Alt+Enter`.
...
- Optional Electron native-window wrapper with single-instance shell-open routing, embedded server startup, packaged-app detection, a disabled-by-default update-check bridge, and an Integration Center launch mode for desktop-lister behavior. Each native launch uses a private random loopback port plus an identity-token health handshake, so it cannot silently attach to an obsolete or unrelated local backend. Packaged launches use the current user's Desktop as their stable workspace root unless an explicit shell-open target or `EXPLORE_BETTER_WORKSPACE_ROOT` is supplied, so launchers cannot leak an unrelated working directory into the lister.
- Supervised persistent Go filesystem helper for complete packaged Windows browsing and Analyzer allocation scans. Cold first paint does not wait for that helper: it streams and stats only the first 48 entries, reports `48+` while the exact total is unknown, then hydrates through the native provider in the background. The bounded slice covers normal viewports while cutting initial stat and DOM work by 40% from the previous 80-row window. Exact local-folder hydration enumerates Windows once, transfers helper metadata as versioned column arrays, and coalesces identical renderer work so matching panes share one download, expansion, label pass, sort, and path index. Pane sorting reuses one numeric, case-insensitive `Intl.Collator` instead of constructing collation options inside every comparison. Later native work reuses the same helper process, and startup begins both pane loads before dormant dialogs or shell-integration diagnostics. Packaged helper lookup prefers the real `resources\\native` executable over the virtual copy inside `app.asar`; `verify:packaged-native-helper` launches the built `.exe`, checks source/package hashes, and requires Analyzer allocation metadata to report the exact Win32 provider. The true-cold 100k browser gate requires the three-run median first visible window within 750ms and complete virtualized hydration within 2s.
...
Large local Windows folders use the bundled normal-user Go helper and one `FindFirstFileW`/`FindNextFileW` pass for names, sizes, timestamps, and attributes. The helper-to-server browse channel uses versioned `columns-v1` arrays to avoid repeating object keys; the browser-facing API hydrates through the dictionary-backed `compact-v2` row format while retaining `compact-v1` compatibility. Small folders, UNC paths, metadata-rich link/dimension views, unsupported filesystems, and helper failures automatically use the bounded concurrent Node provider.

Each pane reports its own navigation state beside the SOURCE/TARGET badge. Large folders move from `Loading` to bounded progress such as `200 / 100K`, then retain the final item count and client-observed elapsed time. The active pane alone owns the global status pill, so a background pane cannot overwrite foreground navigation feedback; existing rows remain usable during hydration and errors preserve the last successful listing.
...
`perf:guard` launches three fresh benchmark processes by default, records every sample, uses the median for numeric budgets and trend history, and requires functional cache/index assertions to pass in all three runs. Set `EB_PERF_GUARD_REPETITIONS=1..7` or `--repetitions=1..7` only when a different certification depth is intentional.
...
npm run verify:packaged-native-helper
npm run verify:native-listing-provider
npm run verify:pane-activity
...
`verify:interaction-resize` is the browser interaction guard for the resizable shell: it drags the navigator, pane splitter, preview, command shelf, and horizontal pane-row handles; proves the compact shelf exposes one action row and a tall shelf exposes at least three wrapped rows while status and modes remain anchored; verifies direct status-control focus/Operations behavior; verifies geometry persistence through `/api/state` and reload; and checks double-click folder open after resizing.
...
Use `Command` or `Ctrl+P` to search built-in file-manager actions, saved trusted tools, and saved trusted scripts from one launcher. Search is fuzzy, so compact initials such as `hsp` can find `Horizontal split panes`. Results rank exact matches first, followed by pinned and recently used actions, then group the complete command registry into task categories. Switch between `All`, `Recent`, and `Pinned`; use the bookmark control or `Ctrl+D` on the active result to pin or unpin it. Arrow keys move the active result, `Enter` runs it, and saved custom hotkeys remain visible beside matching commands. Pins and the latest 12 executed commands are kept locally in the app profile and survive restarts.
...
Use `Toolbar` to choose which built-in command buttons are visible in the middle action zone of the bottom command shelf. Everything remains available from `Command`, but saved toolbar profiles can keep the shelf focused on essentials, organization, or power tools. Drag the shelf taller to wrap more buttons into view; active-pane status remains anchored on the left and workspace modes remain anchored on the right. `Command`, `Prefs`, and `Toolbar` stay fixed so the full command surface is always recoverable.
...
`npm run build:native-helper` builds `native\bin\explore-better-fs.exe`, a Go x64 NDJSON helper with version negotiation, request IDs, progress, cancellation, fast Win32 browsing, enumeration, tree scan, volume geometry, and allocated-size operations. Analyzer uses `GetCompressedFileSizeW` on local Windows volumes and reports `allocatedSource`, `clusterSize`, and `allocationAccuracy`. UNC paths, unsupported platforms, missing helpers, and helper failures remain on the Node fallback. `npm run verify:native-helper` verifies the protocol, browse/allocation metadata, tree totals, and cancellation acknowledgement. `npm run verify:native-listing-provider` compares Win32 browse metadata with Node, validates `compact-v1` and `compact-v2`, then injects a broken helper and proves the Node fallback still returns a complete listing with a diagnostic.
