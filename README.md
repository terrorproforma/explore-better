...
- Supervised persistent Go filesystem helper for complete packaged Windows browsing and Analyzer allocation scans. Cold first paint does not wait for that helper: it streams and stats only the first 80 entries, reports `80+` while the exact total is unknown, then hydrates through the native provider in the background. This slice still covers several screens while cutting startup stat and DOM work by 60% from the previous 200-row window. Exact local-folder hydration enumerates Windows once, transfers helper metadata as versioned column arrays, and coalesces identical renderer work so matching panes share one download, expansion, label pass, sort, and path index. Pane sorting reuses one numeric, case-insensitive `Intl.Collator` instead of constructing collation options inside every comparison. Later native work reuses the same helper process, and startup begins both pane loads before dormant dialogs or shell-integration diagnostics. Packaged helper lookup prefers the real `resources\\native` executable over the virtual copy inside `app.asar`; `verify:packaged-native-helper` launches the built `.exe`, checks source/package hashes, and requires Analyzer allocation metadata to report the exact Win32 provider. The true-cold 100k browser gate requires first visible rows within 750ms and complete virtualized hydration within 2s.
