...
- Crowded tab rows shrink to compact tabs without adding a header scrollbar or leaving buttons off to the side. Explore Better keeps the active tab visible and moves tabs that do not fit into the numbered `...` menu, which contains the complete tab list with names and paths. The menu supports arrow keys, Home/End, Enter, and Escape.
...
- `Archive`: create ZIP files or extract selected ZIPs. Extraction defaults to the folder containing the ZIP and creates a safe sibling folder named after the archive. Right-click a ZIP and choose `Extract Here` to run that default immediately, or use the Archive dialog to choose another target; its `Here` button restores the containing-folder target. Double-clicking a ZIP opens it for read-only browsing in the pane.
- `Sizes`: calculate recursive folder sizes.
- `Disk Map` in the top bar opens a full-window map workspace; `Analyzer` in the command dock opens the complete overview. Both automatically scan the active folder or another path for logical bytes, allocated bytes, drive used/free context, a live scan strip with extension color bands, a folder tree with percent-of-parent bars, a file-type chart with color swatches and categories, top files, and a hierarchical WizTree-style map. The default entry limit is `0` (all entries), so a scan continues until traversal completes or you cancel it; set a positive limit only when you intentionally want a bounded partial scan. Tables, totals, file types, largest files, and the chart update as native progress snapshots arrive. On a volume root, Windows-used space that has not been discovered yet appears as `Not mapped yet`; any residual filesystem metadata or inaccessible use is explicitly retained as `Not enumerable` instead of disappearing from the map. Use the `Map` and `Overview` tabs to reclaim most of the window for the chart or restore all tables without rescanning. `Size by` changes rectangle area between logical and allocated bytes, with allocated bytes as the default for disk maps. `Color by` changes the live legend and chart between file types and stable top-folder branches. Folder rectangles contain their child folders and files, so the chart preserves directory structure instead of flattening every file into one level. Hover a block to inspect its type, logical size, allocated size, and share. Click once to pin a selection; double-click a folder group to focus it in the map, or double-click a concrete file to open its parent and select it in the active pane. `Root`, `Up`, clickable breadcrumbs, `Focus`, and `Open` provide explicit navigation without losing the scan. With the map focused, use the arrow keys to move selection, `Enter` or `Space` to focus/open it, `Backspace` to move up, `Home` to return to the scan root, `O` to open the selection in the active pane, and `Escape` to clear selection. On local Windows volumes the bundled normal-user Go helper performs a memory-bounded native summary traversal and resolves exact allocated bytes without returning millions of file rows to Node. Network, unsupported, or helper-fallback scans are explicitly labeled `Estimated`. Duplicate cold scans of the same target coalesce into one filesystem walk; repeat unchanged scans use a short warm cache and preserve the existing tables and map instead of rebuilding identical UI. App file operations and scripts invalidate matching Analyzer cache entries before the next scan. Use `Active` to load and scan the active pane path, and leave `Follow links` off unless you intentionally want junctions/symlinks traversed.
...
Packaged startup also maintains an existing Explore Better integration. If an older release left an Explore Better-owned PowerShell command behind, startup replaces that legacy command with the current desktop executable. An existing Explore Better folder/drive default remains enabled; a different application's default remains unchanged. Unconfigured shell state is never opted in by this maintenance step.
...
- `npm run verify:shell-current-user` imports the generated shell-handler registry files into the real current-user HKCU shell keys, seeds a legacy PowerShell handler, proves packaged startup repairs it while preserving an Explore Better default, proves a foreign default is not taken over, opens target folders through the direct executable, then restores and compares the pre-trial registry state.
...
- `npm run verify:zip-browse` creates a nested ZIP fixture, browses it through the virtual pane API, verifies the Archive dialog's containing-folder default, runs the real right-click `Extract Here` action, and proves the extracted sibling does not target the opposite pane.
