...
if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
  throw new Error("Usage: node scripts/archive-site-homepage.mjs YYYY-MM-DD");
}

const sourcePath = path.join(root, "site", "index.html");
const archiveName = `legacy-${date}.html`;
...
try {
  ...
  if (!existing.includes(`Archived homepage from ${displayDate}`)) {
...
html = html
  ...
  .replace(
    ...
    `<body class="pitch-home">\n    <aside class="legacy-notice" aria-label="Archived page notice">\n      Archived homepage from ${displayDate}. All original content is preserved here.\n      <a href="index.html">Return to the current homepage</a>\n    </aside>`
  );

if (!html.includes(`Archived homepage from ${displayDate}`) || !html.includes('content="noindex,nofollow"')) {
