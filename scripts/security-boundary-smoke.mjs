...
async function main() {
  ...
  try {
    ...
    const cookie = String(uiResponse.headers.get("set-cookie") || "").split(";", 1)[0];
    check("UI issues a launch capability cookie", cookie.startsWith("ExploreBetterCapability="));
    check("CSP blocks untrusted script sources", String(uiResponse.headers.get("content-security-policy") || "").includes("script-src 'self'"));

    const hostileName = "hostile-origin.txt";
    const hostile = await fetch(`${baseUrl}/api/file/create`, {
      method: "POST",
    ...
    const missingCapability = await fetch(`${baseUrl}/api/file/create`, {
      method: "POST",
    ...
    const authorized = await fetch(`${baseUrl}/api/file/create`, {
      method: "POST",
      headers: {
    ...
    const missingApplyToken = await fetch(`${baseUrl}/api/transfer`, {
      method: "POST",
    ...
    const unsupported = await fetch(`${baseUrl}/api/roots`, { method: "PUT" });
